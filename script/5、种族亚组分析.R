...
# 查看亚组
table(d$Race)
subgroup = c("non-Hispanic white","non-Hispanic black",
...
for (i in 1:length(subgroup)) {
    ...
    fit2 <- svyglm(FI ~ DIIQ + Sex + Age + Educational_level + PIR, 
                   design = nhs.design, family = quasibinomial) |>reg_table()
    fit3 <- svyglm(FI ~ DIIQ + Sex + Age + Educational_level + PIR + 
